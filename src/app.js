const Scriptures = (function () {
  ...
  getScripturesSuccess = function (chapterHtml) {
    document.getElementById(
      DIV_SCRIPTURES
    ).innerHTML = `<div class="chapter-content overflow-auto my-3 shadow bg-superLight rounded pr-4 pl-3 pt-3 pb-6 w-11/12 text-dark">${chapterHtml}</div>`;
  ...
  updateBreadcrumbs = function (bookId, chapter, volumeId) {
    ...
    if (bookId !== undefined) {
      let book = books[bookId];
      let volume = volumes.find((volume) => volume.id === book.parentBookId);
